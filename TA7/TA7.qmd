...
$$\begin{equation}
\begin{split}
\texttt{lwage}_i &= \beta_1 + \beta_2\texttt{exper}_i + \beta_3\texttt{expersq}_i 
\\ &\quad  + \beta_4\texttt{educ}_i + \varepsilon_i
\end{split}
\end{equation}$$
...
$$\begin{equation}
\begin{split}
\texttt{fem_work}_i &= \beta_1 + \beta_2\texttt{other_inc}_i +  \\ &\quad  \beta_3\texttt{fem_educ}_i 
+ \beta_4\texttt{kids}_i + \varepsilon_i
\end{split}
\end{equation}$$

where: 

- $\texttt{fem_work}$: Female labor force participation
- $\texttt{other_inc}$: Other family income (endogenous)
- $\texttt{fem_educ}$: Female education level
- $\texttt{kids}$: Number of kids

But $\text{other_inc}$ is likely endogenous due to omitted variable bias (e.g., family preferences)
...
- $\text{fem_work}$ is a binary outcome (1 if working, 0 otherwise).
...
$$P(\text{fem_work}=1|X) = \Phi(X\beta)$$

The **marginal effect** of one more child on that probability is:

$$\frac{\partial P(\text{fem_work}=1|X)}{\partial \text{kids}} 
